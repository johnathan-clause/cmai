...
local allBotHeroes = {
	...
	---- 'npc_dota_hero_ember_spirit',not sure
	...
	--- 'npc_dota_hero_storm_spirit', weak
	'npc_dota_hero_abaddon',
	---- 'npc_dota_hero_abyssal_underlord',not sure
...
--para mabagal pumili, gametime() >= 35
	if GetHeroPickState() == 60 and GameTime() >= 35 and GameTime() >= lastpick + 2 then
		for i,id in pairs(GetTeamPlayers(GetTeam())) do
			if IsPlayerBot(id) and IsPlayerInHeroSelectionControl(id) and GetSelectedHeroName(id) == "" then
				if testMode then
					hero = GetRandomHero() 
